...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val res: LazyList[(Block, List[Move])] = 
      LazyList((b.left, Left :: history), (b.right, Right :: history), 
      (b.up, Up :: history), (b.down, Down :: history))
    res.filter(_._1.isLegal)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
             if (initial.isEmpty) {
               ...
               val newBlocks = neighbors.map(x => x._1)
               neighbors #::: from(initial.tail ++ neighbors, explored ++ newBlocks)
             }
           }
