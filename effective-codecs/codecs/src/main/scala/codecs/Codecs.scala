...
trait EncoderInstances:
  ...
  given stringEncoder: Encoder[String] =
    Encoder.fromFunction(str => Json.Str(str)) // TODO Implement the `Encoder[String]` given instance
...
trait DecoderInstances:
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction {
      case Json.Obj(fields) if decoder.decode(fields(name)).isDefined => decoder.decode(fields(name)).get
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person](person => new Person(person._1, person._2))
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts](list => new Contacts(list))
...
@main def run(): Unit =
  ...
  // val encoder = Encoder.listEncoder[Person]

  // val encoder = Encoder.fromFunction[List[Person]](as => Json.Arr(as.map(
  //   ObjectEncoder.field[String]("name")
  //     .zip(ObjectEncoder.field[Int]("age"))
  //     .transform[Person](user => (user.name, user.age))
  //     .encode
  ...
  val encoder = Encoder.fromFunction[List[Person]](as => Json.Arr(as.map(
    summon[Encoder[Person]].encode
  )))

  println(encoder.encode(List(Person("vova", 19))))
